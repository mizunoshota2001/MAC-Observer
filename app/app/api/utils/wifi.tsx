...
async function getCurrentSSID() {
  ...
  if (platform === "win32") {
    ...
    wifiInterface = Object.keys(networkInterfaces).find((name) =>
    ...
    wifiInterface = Object.keys(networkInterfaces).find(
      (name) => name.startsWith("wlan") || name.startsWith("wl")
    );
  }
  ...
  const getCurrentConnectionsAsync = promisify(wifi.getCurrentConnections).bind(
    wifi
  );
  ...
  try {
    ...
    throw new Error(
      `ネットワーク情報の取得中にエラーが発生しました: ${error.message}`
    );
  }
}
